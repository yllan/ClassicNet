...
- **憑證驗證 ✅** —— `cn_tls.c` 傳入 CA bundle 時 `VERIFY_REQUIRED`，驗**憑證鏈 + 主機名**。Host 測試（`scripts/test-tls-verify.sh`）3/3：CA 簽署+主機相符→接受、未知 CA→拒絕、主機名不符→拒絕。真機（`cnhttps -DCN_VERIFY=ON`，嵌入測試 CA via `scripts/gen-test-pki.sh`）對 CA 簽署的 server 驗證通過、`status=200`。fail-closed 由 host 證明（同一份 `cn_tls.c`）。
  - ⚠️ **剩餘安全限制**（production 前須處理）：(1) **不檢查有效期** —— cy384 mbedTLS config 關了 `MBEDTLS_HAVE_TIME`，過期/未生效憑證不會因日期被拒（需接 Mac 時鐘 → mbedTLS time）。(2) **熵源品質中等** —— `mbedtls_hardware_poll` 用 Microseconds/GetMouse/TickCount（cy384 自己標註 mediocre），安全性需強化。(3) **CA bundle** —— demo 用拋棄式測試 CA；production 要 bundle 真實 root（Mozilla/curl `cacert.pem`），但 classic Mac RAM 有限，可能要裁剪。
