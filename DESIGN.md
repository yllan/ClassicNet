...
- **憑證驗證 ✅** —— `cn_tls.c` 傳入 CA bundle 時 `VERIFY_REQUIRED`，驗**憑證鏈 + 主機名**。Host 測試（`scripts/test-tls-verify.sh`）3/3：CA 簽署+主機相符→接受、未知 CA→拒絕、主機名不符→拒絕。真機（`cnhttps -DCN_VERIFY=ON`，嵌入測試 CA via `scripts/gen-test-pki.sh`）對 CA 簽署的 server 驗證通過、`status=200`。fail-closed 由 host 證明（同一份 `cn_tls.c`）。
  ...
  - **TLS 1.2 釘死 ✅（production 硬化）** —— `cn_tls.c` 對 mbedTLS 3.x 用 `mbedtls_ssl_conf_max_tls_version` 上限 TLS 1.2，徹底避開 cy384 PPC fork 半殘的 1.3 資料階段（先前 -30082 根因），client 端自鎖、不靠 server 配合。2.28（host）本就 1.2-only，no-op。`CN_TLS_ALLOW_TLS13` 可解開（待完整 1.3 的 PPC mbedTLS）。
  - ⚠️ **剩餘安全限制**（production 前須處理）：**熵源品質中等** —— `mbedtls_hardware_poll` 用 Microseconds/GetMouse/TickCount（cy384 自己標註 mediocre），安全性需強化（見下方 entropy hook）。
